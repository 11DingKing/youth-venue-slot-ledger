...
func (s *Store) AdjustCapacityCommitted(ctx context.Context, slotID, version int64, newCapacity int) (domain.Slot, error) {
	current, err := s.SlotByID(ctx, slotID)
	if err != nil {
		return domain.Slot{}, err
	}
	if current.Version != version || current.Reserved > newCapacity {
		return domain.Slot{}, fmt.Errorf("%w: capacity conflicts with reservations or version", domain.ErrConflict)
	}
