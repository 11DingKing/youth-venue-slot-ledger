...
func (s *Store) AdjustCapacityStandalone(ctx context.Context, slotID, version int64, newCapacity int) (domain.Slot, domain.Slot, error) {
	before, err := s.SlotByID(ctx, slotID)
	if err != nil {
		return domain.Slot{}, domain.Slot{}, err
	}
	adjusted, err := s.AdjustCapacity(ctx, slotID, version, newCapacity)
	if err != nil {
		return domain.Slot{}, domain.Slot{}, err
	}
	return before, adjusted, nil
}
