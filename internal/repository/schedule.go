...
func (s *Store) CreateScheduledSlot(ctx context.Context, slot domain.Slot) (domain.Slot, error) {
	if slot.Status != domain.SlotOpen {
		return domain.Slot{}, fmt.Errorf("%w: scheduled slot must be open", domain.ErrValidation)
	}
	created, err := s.CreateSlot(ctx, slot)
	if err != nil {
		return domain.Slot{}, err
	}
	return created, nil
