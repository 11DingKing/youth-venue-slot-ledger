...
func (s *Store) ReopenClosureRecord(ctx context.Context, id, version int64, at time.Time) (Closure, error) {
	closure, err := s.ClosureByID(ctx, id)
	if err != nil {
		return Closure{}, err
	}
	if closure.Version != version || closure.Status != domain.ClosureApplied {
		return Closure{}, domain.ErrVersionConflict
	}
	return s.TransitionClosure(ctx, id, version, domain.ClosureApplied, domain.ClosureReopened, at)
}
