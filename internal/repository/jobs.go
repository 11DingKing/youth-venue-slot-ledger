...
func (s *Store) EnqueueDurableJob(ctx context.Context, kind, payload, dedupeKey string, availableAt, now time.Time, maxAttempts int) (int64, error) {
	var jobID int64
	err := s.WithTx(ctx, func(tx *Store) error {
		var err error
		jobID, err = tx.EnqueueJob(ctx, kind, payload, dedupeKey, availableAt, now, maxAttempts)
		return err
	})
	if err != nil {
		return 0, fmt.Errorf("persist durable job: %w", err)
	}
	return jobID, nil
}
