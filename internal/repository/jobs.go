...
func (s *Store) ClaimJob(ctx context.Context, now time.Time, lease time.Duration) (domain.WorkerJob, bool, error) {
	...
	err := s.WithTx(claimCtx, func(tx *Store) error {
		row := tx.q.QueryRowContext(claimCtx, `SELECT id, kind, payload, status, attempts, max_attempts,
		...
		result, err := tx.q.ExecContext(claimCtx, `UPDATE worker_jobs SET status = 'running', attempts = attempts + 1,
