...
func (s *Store) SaveCommittedIdempotencyRecord(ctx context.Context, actorID int64, method, path, key, requestHash, resourceType string, resourceID int64, expiresAt, now time.Time) error {
	existing, found, err := s.IdempotencyRecord(ctx, actorID, method, path, key, now)
	if err != nil {
		return err
	}
	if found {
		if existing.RequestHash != requestHash || existing.ResourceType != resourceType || existing.ResourceID != resourceID {
			return domain.ErrIdempotencyReuse
		}
		return nil
	}
	return s.SaveIdempotencyRecord(ctx, actorID, method, path, key, requestHash, resourceType, resourceID, expiresAt, now)
}
