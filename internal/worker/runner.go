...
func (r *Runner) HandleOnce(ctx context.Context) (bool, error) {
	...
	now := r.now().UTC()
	job, found, err := r.store.ClaimJob(workCtx, now, r.lease)
	if err != nil || !found {
		return false, err
	}
	r.mu.RLock()
	...
	if err == nil {
		if completeErr := r.store.CompleteJob(workCtx, job.ID, completedAt); completeErr != nil {
	...
	if failErr := r.store.FailJob(workCtx, job, err, completedAt, backoff); failErr != nil {
